...
Дорога эта долгая, и по ней уезжает необязательный колбэк progress: каждый шаг сообщает о
себе структурой karachur.gemini.stages.Stage, а кто и как покажет ее человеку - забота
karachur.tg.status. Без колбэка (progress=None) все работает ровно как раньше.
"""
...
async def generate_gemini_response(
    ...
    await stages.report(progress, stages.Stage(stages.CONTEXT))
    context_messages, summary = await compress.compress_context(
        cfg, pool, conn, chat_id, context_messages, summary, progress
    )
    ...
    response_text = await retries.generate_with_retries(
        cfg, pool, make_contents, progress
    )
