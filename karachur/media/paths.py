...
уходит слэш, поэтому вложение не может увести путь за пределы каталога медиа. Само
original_name при этом не уникально: два разных документа в чате вполне могут называться
report.pdf, а download_media_file существующий файл повторно не качает - поэтому к
читаемому имени подмешивается file_id (report.<file_id>.pdf), который как раз уникален.
Все остальные случаи - имени нет или оно не ascii - собирают имя из file_id и расширения,
выведенного по mime: file_id уникален сам по себе, и гадать с ним не о чем.
...
def get_media_path(
    ...
    if original_name and original_name.isascii():
        ...
        # file_id вклинивается между именем и расширением: два разных документа с
        # одинаковым original_name (например, report.pdf) иначе легли бы по одному и
        # тому же пути, а download_media_file существующий файл повторно не качает -
        # второй файл в чате получил бы содержимое первого. Имя при этом остается
        # читаемым, только с добавкой между основной частью и расширением.
        stem, dot, ext = safe_name.rpartition(".")
        if dot:
            safe_name = f"{stem}.{file_id}.{ext}"
        else:
            safe_name = f"{safe_name}.{file_id}"
