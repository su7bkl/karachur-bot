...
Отсюда и разделение пакета:
    paths     - куда файл ляжет на диске и как он туда скачается из Телеграма
...
Наружу торчит одна функция - normalize: karachur.tg.handlers зовет ее сразу после
скачивания файла и запоминает, что она вернула. Возврат mime=None - это "модели не
отдавать": обработчик грузит файл только при непустом mime, и пропуск архива выражается
именно так.

Саму policy при этом спрашивают и в обход normalize, с двух сторон от нее. До
скачивания - karachur.tg.handlers (policy.decide_without_content): архив незачем даже
качать. Перед выгрузкой в Files API - karachur.gemini.contents (policy.is_supported):
нормализация могла не справиться, а неподготовленный файл модель отвергает ошибкой,
после которой чат перестает отвечать вообще.
