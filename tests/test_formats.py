...
чтобы не зависеть от машины. Третий - media.normalize целиком: именно ее зовет bot.py, и
именно ее контракт (путь, mime) не должен меняться.
...
def test_document_comes_back_from_normalize_as_pdf(tmp_path):
    ...
    Это и есть весь смысл затеи: bot.py зовет только normalize, и конвертация документов
...
def test_archive_is_not_offered_to_the_model(tmp_path):
    """У архива mime обнуляется - bot.py грузит файл только при непустом mime."""
